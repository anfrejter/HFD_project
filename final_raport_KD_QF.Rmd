...
**The parameter optimization process was conducted using Optuna in Python**, a hyperparameter optimization framework. The search involved defining a parameter space for moving averages, volatility windows, and thresholds, with each strategy's performance evaluated as an objective function. Optuna dynamically adjusted parameters during trials to explore high-performing combinations, and results were logged for detailed post-optimization analysis.

Objective function was initially defined as below:

$\sum_{i = q_{1}}^{q_{n}} stat_{i}$

Where $stat_{i}$ is the statistic of the strategy for quarter $q_{i}$.

However, strategies optimized using this function were prone to overfitting. To address this, we revised the objective function to:

$\frac{\sum_{i=1}^n stat_{i}}{n} - \frac{1}{2}\sqrt{\frac{\sum_{i=1}^n (stat_{i} - \bar{stat})^2}{n-1}}$

This new function incorporates a penalty for standard deviation, weighted by $\frac{1}{2}$, to balance performance consistency with high returns.

## Hyperparameter Optimization Workflow

Hyperparameter optimization was performed using the Python script `run_hp_tuning.py`. This script integrates parameter selection, strategy execution, and evaluation into a streamlined process. The key tasks performed by the script are as follows:

1.  **Parameter Selection with Optuna**:\
    Optuna dynamically selects hyperparameters to test during each trial.

2.  **Execution of R Scripts**:\
    The script calls R functions with the selected parameters to execute various strategies, including:
...
3.  **Evaluation of Results**:\
    After running the strategies, the results from the trial are evaluated.

The process loops back to step 1, with Optuna refining the parameter selection, until the predefined number of trials has been completed.

# Group 1 -- summary of results (including out-of-sample)

## Finally selected strategy for **group 1**

The Volatility Breakout (VB) strategy was selected for Group 1 assets, focusing on a single instrument, NQ. This asset has a point value of \$20 and a transaction cost of \$12. The strategy identifies price breakouts by utilizing a combination of moving averages: MA57 (fast) and MA114 (slow). The volatility memory window is set to 68, allowing the strategy to adjust dynamically to recent volatility trends.

The breakout multiplier is set to 2.66573, which is relatively wide, suggesting that multiple transactions are likely to occur. Price signals are smoothed using a mean estimator, helping to filter out noise and enhance the quality of trade signals. Regime detection is carried out using rolling windows, with a `window_regime` value of 40. Additionally, a dynamic, regime-based approach (fl) is employed, enabling the strategy to adapt to shifting market conditions.
...
The **Gross Calmar Ratio** and **Net Calmar Ratio** reached extraordinary highs in **2022_Q4**, with **grossCR** at **70,789.95** and **netCR** at **905.10**, signaling excellent returns relative to drawdowns. However, in subsequent quarters, these ratios fell sharply, especially in the latter half of 2023 and into 2024.

The **stat** reaches its peak in **2022_Q4** (2,187.12), correlating with high profitability, while very low values, like -6.17 in 2023_Q4, indicate poor performance. Additionally, **2022_Q4** saw extraordinarily high Gross and Net Calmar Ratios, reflecting exceptional returns relative to drawdown during that period.
...
The chart shows the cumulative Net and Gross PnL over the period July 3, 2023, to September 29, 2023 created based on out of sample data. At the start of the period (early July), both Net and Gross PnL fluctuate around negative values, reflecting initial losses.Then, at the end of July, gross PnL increased to more than 6000. Shortly after,it decreased to values around 2000. Then our strategy remains relatively flat with visible drop at the end of the period. Finally, Net PnL reached value of over -1,500 net.
...
The Double Volatility Breakout (VB) strategy was selected for Group 2 assets. This strategy involves trading two instruments: XAG, with a point value of \$5000 and a transaction cost of \$10, and XAU, with a point value of \$100 and a transaction cost of \$15. The strategy identifies price breakouts using a combination of moving averages and volatility parameters.

For XAG, the moving averages are configured as MA51 (fast) and MA140 (slow), while for XAU, they are set to MA58 (fast) and MA115 (slow). Volatility parameters are set to 72 for XAG and 73 for XAU, respectively.

Price signals for XAG are smoothed using a mean estimator, while a median estimator is applied for XAU. Regime detection is performed using rolling windows, with `window_regime = 98` for XAG and `window_regime = 22` for XAU. Entry and exit thresholds for both instruments are dynamically adjusted: XAG uses an entry multiplier of 3.63864 and an exit multiplier of 1.79056, while XAU uses an entry multiplier of 2.026 and an exit multiplier of 0.10742.

The entry multiplier for XAG is relatively wide, which means we expect trades to occur mainly during significant price movements. In contrast, the exit multiplier for XAU is very narrow, implying a very short holding period. These multiplier settings may indicate overfitting to specific high-profit trades with low drawdown, likely tailored to a few optimal scenarios.
...
### Group 1 Strategy Performance

The strategy applied to **Group 1 data** achieved an overall **positive net return** of approximately **20,700** on in-sample data. However, the profits were **inconsistent**—the majority of gains were generated during two months, while the remaining months saw losses. This suggests that the strategy may have **overfitted** to a few highly profitable trades, rather than exhibiting sustainable performance across the entire period.

-   The **Sharpe Ratio (SR)** ranges from **-3.08** in **2023_Q4** to a high of **5.76** in **2022_Q3**. Higher SR values suggest better risk-adjusted returns, with **2022_Q3** standing out as the most profitable quarter.

-   **Trade Frequency**: On average, around **2-3 trades per day** were made, with one quarter experiencing as many as **12 trades per day** on average.

-   **Out-of-sample Data**: Positive net PnL was observed in only **two quarters**, while the rest were highly unprofitable. This further confirms the hypothesis that the strategy might have overfitted to the in-sample data. Overall net profit on out of sample data was around 10,300.

### Group 2 Strategy Performance

The strategy applied to **Group 2 data** generated a **net profit of 20,900** on in-sample data, though performance was not entirely consistent. The majority of the profit was concentrated in **2024_Q2**, which contributed significantly to the overall net PnL for those quarters.

-   The **Sharpe Ratio (SR)** for **Dataset 2** follows a similar trend, with positive values in quarters like **2022_Q3** (**2.59**) and **2022_Q4** (**2.63**), but declining significantly in later quarters like **2023_Q3** (**-2.58**) and **2024_Q3** (**-2.67**)

-   **Trade Frequency**: On average, fewer than **one trade per day** was executed, with the highest average number of trades occurring in **2023_Q2**, at **3.8 trades per day**.

<!-- -->

-   **Out-of-sample Data**: Four out of five quarters were **highly unprofitable**, reinforcing the idea that the strategy may have overfitted to the in-sample data. Overall net loss on out of sample data was around -19,100.

### Key Insights

-   Both strategies showed **positive overall returns**, but with notable inconsistencies.

-   In both groups, the good performance measured by net SR and net PNL were concentrated in a few specific quarters, while some quarters experienced losses.

-   The performance of both strategies highlights how **overfitting** to a small number of highly profitable trades can affect **long-term sustainability**.

-   **Trade Frequency vs. Profitability**: While trade frequency varied, there was no consistent correlation with profitability. Some of the high-frequency trading periods were associated with substantial losses, reinforcing the importance of strategy robustness over frequent trading.
