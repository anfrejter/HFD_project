...
-   **Dynamic Regime-Based Strategy**: This adaptive approach switches between momentum and mean reversion strategies based on the detected market regime. The market regime is determined using rolling windows and thresholds, with the type of regime (momentum or reversal) detected through the **correlation** of price movements. If the correlation calculated over a specified window is positive and close to 1, we expect momentum behavior. On the other hand, if the correlation is negative, we expect mean-reverting behavior.
...
After the first quarter, only a few trades were executed, with the final one being the only profitable trade. However, the overall PnL ended up around -1000. \## PnL of results for **group 2** -- quarter 2022Q2 -- out of sample
